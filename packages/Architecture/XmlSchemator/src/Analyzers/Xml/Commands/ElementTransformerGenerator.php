...
use Architecture\XmlSchemator\Analyzers\Xml\Helpers\TransformerBuilder;
...
class ElementTransformerGenerator extends GeneratorCommand
{
    const FILE_TYPE = 'Transformer';
    const SHORT_INDENT = '    ';
    const LONG_INDENT = '        ';
     ...
     * @var string
     */
    protected $name = 'core:xml:generate:transformer:node:element';
    /**
     ...
     */
    protected string $pathStructure = '{SectionName}/{ContainerName}/UI/API/Transformers/{VersionNumber}{PathFolder}/*';
    /**
    ...
    public function handle()
    {
        ...
        $this->printInfoMessage('Generating Transformer');
        ...
        $this->baseNamespace = new NamespaceBuilder(
            ...
            context: Str::of(Self::FILE_TYPE)->plural()->start('UI\\API\\'),                        // \UI\API\Transformers
            version: $this->contextInput->get('xsd-version-number')->present()->asStudlyFormat(),
            path: Str::of(Arr::get($parent, 'classNamespacePath'))
        );

        $modelBuilder = TransformerBuilder::fromContextInput($this->contextInput);

        $attributes = Self::buildAttributes($this->contextInput, $this->baseNamespace);
        ...
        $generatorParams = [
            ...
            'stub-parameters' => [
                ...
                'collection-name'                    => TransformerBuilder::getDbCollectionName($this->contextInput),
    ...
    /**
     * [buildAttributes description]
     * @param  [type] $contextInput [description]
     * @return [type]               [description]
     */
    protected static function buildAttributes($contextInput)
    {
        $model = $contextInput->get('model')->present()->asSnakeFormat();
        ...
        // get columns from NodeAttributes
        $obj = 'App\\Containers\\' . $section . '\\' . $container . '\\UI\\API\\Transporters\\' . $model;
        $obj = new $obj();
        ...
        foreach ($fields as $key => $value) {
            $attributes .= Self::SHORT_INDENT . Self::LONG_INDENT . "'$key' => $value," . PHP_EOL;
        }
