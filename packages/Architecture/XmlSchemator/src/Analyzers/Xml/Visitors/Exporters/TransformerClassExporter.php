...
use Architecture\XmlSchemator\Analyzers\Xml\Directors\Traits\SetsNodeElementTransformerBuilder;
 ...
 */
class TransformerClassExporter implements ExporterVisitorContract
{
    use SetsNodeElementTransformerBuilder;
    ...
    {
        $this->setElementNodeTransformerBuilder(new Models\ElementCommandBuilder());
    }

    public function export(XmlNodeValue $xmlNode)
    {
        match ($xmlNode::NODE_NAME) {
            NodesEnum::XS_ELEMENT      => $this->buildElementNodeTransformerClass($xmlNode),
            default => null,
        };
    }

    public function buildElementNodeTransformerClass(ElementNodeValue $elementNode)
    {
        ...
        return $this->elementNodeTransformerBuilder
            ->reset()
